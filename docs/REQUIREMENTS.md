...
- **Placement** (Phase 4b PR #13, then Phase 4b-REDO per playbook trap T8): all 249 footprints positioned via connectivity-driven analysis. 205 on F.Cu, 44 on B.Cu. Per-MCU rotation locks each chip's PWM corner (RIGHT+BOTTOM of chip) toward its channel's MOSFET sub-grid (CH1 θ=0°, CH2 θ=90°, CH3 θ=270°, CH4 θ=180°). FETs re-grouped from row-per-channel (60 mm spread) into 3×2 corner sub-grids (25×13 mm cluster). Mount holes corrected to 4 at proper corners (5,5)/(80,5)/(5,65)/(80,65). T7 connector accessibility verified. Heatsink zone (80 × 55 mm B.Cu center over the 6×4 grid) unchanged — Phase 4c thermal verdict preserved. See `docs/PHASE4B_REDO_PLACEMENT.md`.
