...
- **Form factor**: **90 × 75 mm rectangular** (re-locked at Phase 4b-REDO2 per master adjudication 2026-05-22 to absorb BEC expansion from Phase 2d-REDO + 2e-REDO). Size history: 50×50 (initial) → 85×70 (Phase 4c-resume Option C, for 24× TOLL MOSFETs) → 90×75 (this phase, +13.4% area for BEC absorption; 5 bucks + LC + safety + 16 solder pads = ~1680 mm² new content). 90×75 F.Cu utilization ~62% — comfortable for routing per FPV-density discipline.
...
- **Edge.Cuts outline**: 90 × 75 mm rectangular. Edge.Cuts layer stroke 0.05 mm.
- **Mounting**: 4× M3 holes on custom **80 × 65 spacing pattern** (corners at (5,5), (85,5), (5,70), (85,70) on 90×75 board). No standard FPV-stack fit — commercial-product-class custom pattern per `feedback-anchor-on-most-capable-reference` rule.
