...
- **Cooling solution thermal commitment (Phase 7-prep, added 2026-05-23 per Phase 4 thermal sim refinement)**: B.Cu-side cooling MUST achieve **h_bot ≥ 1500 W/m²·K typical** at FET pad locations. This is anchored on Phase 4 Elmer FEM Sim 1 verification (per-FET T_J ≤ 100°C continuous / ≤ 150°C burst at h_bot=1500). Mechanical design must commit to: (a) thermal interface material (TIM) with k ≥ 4 W/m·K and ≤ 0.5mm thickness, (b) heatsink mass + fin geometry adequate to maintain h ≥ 1500 W/m²·K under enclosure airflow. If mechanical analysis (Phase 7-prep) cannot achieve this h_bot, MOSFETs must be re-spec'd (lower R_DS(on)) OR de-rated I_max (lower I² loss). Root-cause prevention rule: thermal sim methodology now anchored on Phase 7-prep mechanical commitment (no isolated PCB-only thermal claims).
- **Edge.Cuts outline**: **100 × 95 mm rectangular** (Sai-approved 2026-05-23 PR-A4-a board grow from 100×85). Rationale: 4-channel template instantiation with AOTL66912 TO-263 P=12mm FET pitch requires CH3/CH4 SW/SE quadrants ≥32mm tall — 100×85 was 10mm short. Edge.Cuts layer stroke 0.05 mm.
- **Mounting**: 4× M3 holes on custom **90 × 85 spacing pattern** (corners at (5,5), (95,5), (5,90), (95,90) on 100×95 board — PR-A4-a 2026-05-23). No standard FPV-stack fit — commercial-product-class custom pattern per `feedback-anchor-on-most-capable-reference` rule.
