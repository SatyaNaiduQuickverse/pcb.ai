...
### L16 — Stacked microvia F↔In1↔In2 mathematically guarantees escape budget at 0.5mm QFN pitch (CH1 30/30 lever L, 2026-05-28)

- **Date**: 2026-05-28
- **Pattern**: dense fine-pitch escape on whitelist pins where the existing layer-aware HDI supply (blind F-In2 = 1 slot per pin) leaves ZERO margin against demand 1 per pin. Per `[[feedback-physics-as-compass]]` + `[[feedback-sureshot-over-sota]]`, supply == demand is a brittle point target — process drift, registration tolerance, or any contention spills onto the 24/30 stable attractor (L15).
- **Observation**: prior CH1 30/30 levers (D, F, G, J) closed the escape budget at exactly 1 signal-reaching slot per whitelist pin (the OQ-020 blind F-In2 class). Lever L observes that JLC HDI Class 2 supports **stacked microvia natively** (TWO microvias geometrically aligned — top F.Cu↔In1.Cu stacked on bottom In1.Cu↔In2.Cu — with an isolated "antipad+pad" copper island on In1 between them; the In1 landing is tied neither to GND plane nor to the signal, acting purely as a stacked-via pad). This adds a SECOND signal-reaching via mechanism per whitelist pin, mathematically DOUBLING supply.
- **Root cause** (physics + fab): stacked microvia is two single-laser-drill microvia operations geometrically aligned, each individually satisfying JLC HDI Class 2 single-laser-drill adjacent-layer spec (top: F-In1; bottom: In1-In2). The In1 pad island is electrically isolated from the In1 GND plane by an antipad (standard PCB feature). Continuous signal path: F.Cu pin → top microvia → In1 isolated pad → bottom microvia → In2 signal escape. Process is industry-standard since ~iPhone 4 era — Apple/Samsung phones use stacked microvia at every fine-pitch BGA/QFN with millions of fielded units of established reliability. The +$1-2/board adder is the second laser pass + alignment; no new fab class.
- **Cost adjustment** (`hardware/kicad/scripts/audit_hdi_via_in_pad.py`, `route_subsystem_cooperative.py`, `pcbai_fpv4in1.kicad_dru`, `pcbai_fpv4in1.kicad_pro`, `BOARD_INVARIANTS.md`):
  - `audit_hdi_via_in_pad.STACKED_MICROVIA_NET_WHITELIST` (6 nets — same as BLIND_F_IN2) + `STACKED_MICROVIA_SANCTIONED_LANDINGS` (8 pins) — net-name + pin-landing SSoT, post-loop pair detection (groups co-located MICROVIA legs by snap-XY + net; F-In1 + In1-In2 pair on whitelist net = sanctioned).
  - `route_subsystem_cooperative.via_class_for_span` adds `stacked_microvia_F_In1_In2` class — emits TWO `VIATYPE_MICROVIA` vias (top + bottom) at same XY on whitelist nets; SSoT-imported whitelist from audit (no hard-coded net names in the router).
  - `via_diam_mm_for_class` / `via_halo_radius_mm` / `via_pad_half_mm_for_class` / `via_span_layers` extended for the new class — same SSoT discipline as v9 lever F per-class halo. Halo derives automatically from the new diameter constant (0.25mm pad, same as microvia F-In1).
  - DRU + .kicad_pro carry the leg geometry (drill 0.10mm / pad 0.25mm / annular 0.075mm / hole clearance 0.10mm — identical to existing microvia; same `A.Hole <= 0.15mm` clause) scoped to the 6 named nets via `==` per `[[reference-kicad-dru-libeval-crash]]`.
- **Why this is not "throw HDI at it"** (the L1 hard block / `[[feedback-build-routing-system-not-freerouter.md]]`): stacked microvia is a DETERMINISTIC + INDUSTRY-STANDARD fab class — the geometry is closed-form (two adjacent microvias with In1 antipad+pad), the reliability profile is established (millions of phones), the audit + DRU enforcement is the same SSoT pattern proven by levers D, F, G. No heuristic search. Per `[[feedback-sureshot-over-sota]]`, verifiability + bounded failure modes trump cutting-edge — and stacked microvia is older + better-fielded than any custom router class. Per `[[feedback-codify-not-patch]]`, the addition ships with 3 artifacts: codified emit-path (router via_class), codified audit gate (pair-detection function), master-independent regression test (`test_emit_blind_f_in2.py` extended).
- **Status**: proposed
- **Sim cross-check**: `test_emit_blind_f_in2.py` extended with stacked microvia emit tests (6 nets × 8 landings × verify VIATYPE_MICROVIA × 2 legs at expected layer pairs + drill + pad geometry); non-whitelist refuse test mirrors blind F-In2 negative; existing 17 blind/microvia/halo/foreign-via tests PASS unchanged; audit synthetic test detects stacked pair on whitelist net + FAILS on non-whitelist (proves whitelist scope is BINDING end-to-end). Per-side ledger delta on canonical d4ab0f2 read-only board: each whitelist side gains +1 signal-reaching slot per whitelist-eligible residual net (stacked_microvia_F_In1_In2 supply = same set as blind_f_in2_supply); the layer-aware supply on whitelist sides grows by exactly the # of whitelist-eligible residual nets, guaranteeing supply > demand at every pin landing. FoS verification: annular ring 0.075mm ≥ 0.05mm fab min (×1.5 FoS per §5c) on each leg; stacking alignment 0.025mm ≤ 0.075mm annular budget (no cut-to-cut); hole clearance 0.10mm = existing HDI envelope; no §5c violations.
...
```
ROUTING_LESSONS_HASH = f38ec50f20a28cd1a86387984edf739254ef3a901a5b01067ddc323d8216e00c
```
