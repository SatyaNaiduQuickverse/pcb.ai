...
| R42 | **Targeted leaf-route provenance + cascade-bound + shorts-gate** — every targeted-leaf-route attempt by the cooperative router's `run_leaf_route_phase` (lever Q) on a committed-but-partial net (verify_net_connectivity > 1 island) MUST write a provenance entry under `sims/routing_provenance/leaf_route/`; cascade_attempts ≤ `LEAF_ROUTE_ATTEMPT_CAP = 2` (maze → multi-mech); every COMMITTED entry MUST satisfy `shorts_post − shorts_pre ≤ 0` (the R-J5 SHORTS DELTA invariant extended to leaf-route — no second-class commits); `committed=True` ⇒ `final_outcome=ROUTED` and vice-versa (no fabricated success on a rolled-back attempt). | `route_subsystem_cooperative.py` (CH1 30/30 lever Q, this PR) — `_lq_attempt_leaf_route` + `_lq_run_leaf_route_phase` + `route_unconnected_leaves(board, net_name)` public API; `LEAF_ROUTE_ATTEMPT_CAP = 2` constant is the code-side SSoT; consumes existing find_path_astar (mechanism a) + `_try_multi_mech_fallback` (mechanism b) — no SSoT bypass; CLI `--enable-leaf-route` flag; READ-ONLY on `.kicad_pcb` (caller persists via existing `pcbnew.SaveBoard`) | `audit_leaf_route_provenance.py` G_Q1 (master, this PR): scans provenance dir, asserts schema (netname/leaf_pad/attempts non-empty/cascade_attempts in [1, 2]/shorts_pre,shorts_post non-negative/final_outcome in {ROUTED, NO_PATH, SHORTS_GATE_REJECT, DISABLED}/committed-vs-outcome consistency), FAILs on cascade > cap OR shorts-delta > 0 on commit OR ROUTED-without-commit OR commit-without-ROUTED | 2026-05-29 master: `test_leaf_route.py` 20/20 PASS — well-formed entries accepted + 4 adversarial classes (shorts-delta-positive liar, committed-but-NO_PATH liar, ROUTED-without-commit liar, missing-shorts liar) all REJECTED by G_Q1; vacuous-PASS on no-leaves-found canonical state | ✅ **NEW Sai 2026-05-29 (CH1 30/30 lever Q)**: PR #227 + K2 still leaves the case where a partial-MST net commits an N−1 trunk + 1 disconnected leaf with `no_conflict` lever-J verdict (no foreign track on the ideal corridor — the obstacle is a foreign-via halo K2's full-multi-source rejoin still couldn't bridge inside ONE MST call). KILL_RAIL_N R76.1 canonical example. Lever Q closes the gap with a per-leaf bounded mechanism cascade (maze → multi-mech) that runs AFTER the cooperative + lever-J phases on the SAME board obstacle map. Per `[[feedback-codify-not-patch]]` + `[[feedback-sureshot-over-sota]]` + `[[feedback-deliver-not-promise]]`. |
...
| 15 | orphan-via class (through-via with no same-net Cu contact on EITHER surface layer; flagged as `via_dangling` by kicad-cli) — surfaced on canonical post-T (085dee9) as 3 residual offenders on MOTOR_A_CH1@(18,54), MOTOR_C_CH1@(18,82), SHUNT_C_TOP_CH1@(10,86) that lever T's +VMOTOR pour fix couldn't rescue (wrong nets). Root cause: upstream via-grid generators (add_sw_vias.py, route_*.py) admit candidates that fit inside ANY same-net F.Cu polygon AND any B.Cu polygon (pad OR pour bbox), but ZONE_FILLER on the refilled board pulls the pour back by the foreign-net clearance, stranding pre-existing vias in pour-clearance void. The in-memory synthetic tests in `test_inject_vmotor_pour.py` missed this because they tested only the +VMOTOR-net case, not arbitrary foreign-net through-vias. | 2026-05-29 Sai-catch (PR #251 lever V) | `inject_vmotor_pour.py` Stage 5 = post-refill `kicad-cli pcb drc` parse + orphan-via removal (any net) + iterative re-verify (max 3 iters, bail-out on non-match) + G14 +VMOTOR density floor (4/cm²) sanity check | (a) `test_inject_vmotor_pour_live.py::test_V2_canonical_post_T_3_residual_to_zero` — canonical-load gate (per `[[feedback-sim-artifact-must-be-canonical]]` analogue: tool MUST be proved against the canonical artifact, not just synthetic). (b) Synthetic-mirror tests V1/V3/V4/V5 cover Stage 5 logic without the 8MB canonical fixture. (c) `report['stage5_orphan_via_cleanup']['post_dangling_count']` MUST equal 0 — tool exits 1 otherwise. | 2026-05-29 master PR #251 | ✅ **NEW gate added**; 3 orphans removed on canonical post-T; G14 density 4.45/cm² remains above floor; verifies via kicad-cli ground truth. Codifies `[[feedback-sai-catches-are-samples]]`: the next layer-aware via tool should run the same Stage 5 pattern as a tail-pass (extract `cleanup_orphan_vias()` into a shared helper for reuse). |
