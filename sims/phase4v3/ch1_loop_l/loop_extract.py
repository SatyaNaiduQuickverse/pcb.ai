...
WHAT IT DOES
------------
Reads the PLACED board (/tmp/ch1_152.kicad_pcb) via pcbnew and extracts, from REAL
...
BOARD_PATH = "/tmp/ch1_152.kicad_pcb"
...
def main():
    ...
    for ph, (hs, ls, swnet, srcnet, shnet) in phases.items():
        ...
        # Plane-referenced parametric term L_plane(d) = mu0*d*l/w. d undefined in board;
        # report across documented JLC 8L/1.6mm prepreg range (NOT fabricated as a single
        # value). This is the post-route/post-stackup-lock estimate basis.
        d_lo, d_hi = 0.0762e-3, 0.21e-3  # JLC 8L/1.6mm F.Cu->In1 prepreg span (3/4mil..)
        Lpl_lo = MU0 * d_lo * (l_run_mm * 1e-3) / (w_span_mm * 1e-3) * 1e9
        Lpl_hi = MU0 * d_hi * (l_run_mm * 1e-3) / (w_span_mm * 1e-3) * 1e9
        ...
        print(f"    [PLACEMENT-STAGE, geometry-only worst case] L_loop = {L_loop:.4f} nH  "
              f"margin={margin:+.4f}nH -> {verdict}")
        print(f"    [plane-referenced, post-stackup-lock] L_via_cluster + L_plane(d) "
              f"= {L_via_cluster+Lpl_lo:.4f}..{L_via_cluster+Lpl_hi:.4f} nH "
              f"(d=0.076..0.21mm JLC 8L/1.6mm prepreg; UNCONFIRMED, dielectric not in board)\n")
